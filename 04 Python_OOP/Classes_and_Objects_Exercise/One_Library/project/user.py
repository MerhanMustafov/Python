...
class User:
        ...
        if book_name in library.books_available[author]:
            ...
            for k, v in library.rented_books.items():
                for k1, v1 in v.items():
                    if k1 == book_name:
                        return f'The book "{book_name}" is already rented and will be available in {v1} days!'
        ...
        if book_name in self.books:
            ...
            for k, v in library.rented_books.items():
                if book_name in v.keys():
                    del library.rented_books[k][book_name]
                    break
    ...
    def info(self):
        self.books.sort()
        return ", ".join([book for book in self.books])
